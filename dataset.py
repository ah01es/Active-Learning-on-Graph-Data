...
def get_dataset(name, data_dir):
    ...
    if name == 'EMNIST':
        ...
        return get_Cora(data_dir)



# def get_Cora(data_dir):
#     cora_path = os.path.join(data_dir, 'Cora')
...
#     X_tr = X[data.train_mask]  
#     Y_tr = Y[data.train_mask]  
#     X_te = X[data.test_mask]   
#     Y_te = Y[data.test_mask]   

#     return X_tr, Y_tr, X_te, Y_te


def get_Cora(data_dir):
    dataset = Planetoid(root=os.path.join(data_dir, 'Cora'), name='Cora')
    data = dataset[0]

    X_tr = data.x[data.train_mask]  
    Y_tr = data.y[data.train_mask]  
    X_te = data.x[data.test_mask]  
    Y_te = data.y[data.test_mask]  

    num_extra_train = 500
    test_indices = torch.where(data.test_mask)[0]
    selected_indices = torch.randperm(len(test_indices))[:num_extra_train] 
    
    extra_X_tr = X_te[selected_indices]
    extra_Y_tr = Y_te[selected_indices]

    remaining_indices = torch.tensor([i for i in range(len(X_te)) if i not in selected_indices])
    X_te = X_te[remaining_indices]
    Y_te = Y_te[remaining_indices]

    X_tr = torch.cat([X_tr, extra_X_tr], dim=0)
    Y_tr = torch.cat([Y_tr, extra_Y_tr], dim=0)

    print(f"New Training samples: {len(X_tr)}")
    print(f"New Test samples: {len(X_te)}")
...
#         os.makedirs(mnist_path, exist_ok=True)  # ایجاد پوشه اگر وجود نداشته باشد
...
#             download_and_extract_archive(url, download_root=mnist_path)

#     print(f"MNIST data path: {mnist_path}")
#     raw_tr = datasets.MNIST(root=os.path.join(data_dir, 'MNIST'), train=True, download=False)

#     raw_tr = datasets.MNIST(root=os.path.join(data_dir, 'MNIST'), train=False, download=False)

#     X_tr, Y_tr = raw_tr.data, raw_tr.targets
#     X_te, Y_te = raw_te.data, raw_te.targets
