...
class AlphaMixSampling(Strategy):
	...
	# def sample(self, n, feats):
	# 	feats = feats.numpy()
	# 	cluster_learner = KMedoids(n_clusters=n, metric='euclidean', init='k-medoids++', random_state=42)
	# 	cluster_learner.fit(feats)
	# 	cluster_idxs = cluster_learner.labels_
	# 	centers = cluster_learner.cluster_centers_
	# 	kde = KernelDensity(kernel='gaussian', bandwidth=0.5).fit(feats)
	# 	densities = kde.score_samples(feats)
	# 	dis = np.linalg.norm(feats - centers[cluster_idxs], axis=1)
	# 	scores = dis - densities
	# 	return np.array(
    #         [np.arange(feats.shape[0])[cluster_idxs == i][scores[cluster_idxs == i].argmin()] for i in range(n) if
    #          (cluster_idxs == i).sum() > 0])
